 ...
 * ' upcas r1 r2 r3 'which puts r3 into m[r1] if m[r1] == r2
 * It then puts into the swap register the value of m[r1] before the swap
 * @author amit
 */
public class UpdateCompareAndSwapInstruction extends AbstractInstruction implements MemoryInstructionInterface {
    ...
    public UpdateCompareAndSwapInstruction() {
    }

    public UpdateCompareAndSwapInstruction(int instruction) {
        this();
    ...
    public AbstractInstruction getAbstractInstruction(int instruction) {
        return new UpdateCompareAndSwapInstruction(instruction);
    }
    ...
    public MemoryInstruction getMemoryInstruction(RegisterFile rFile) {
        ...
        toRet.setSubType(SubTypes.UpdateStore);
        return toRet;
    }
    ...
    public static void main(String[] args) {
        ...
        UpdateCompareAndSwapInstruction c = new UpdateCompareAndSwapInstruction();
        int instruction = c.assembleInstruction(instr);
        AbstractInstruction.getAbstractInstructionForInstruction(instruction).toString();
        c = (UpdateCompareAndSwapInstruction) c.getAbstractInstruction(instruction);
