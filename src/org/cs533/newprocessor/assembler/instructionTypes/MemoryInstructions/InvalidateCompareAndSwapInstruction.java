 ...
 * ' ivcas r1 r2 r3 'which puts r3 into m[r1] if m[r1] == r2
 * It then puts into the swap register the value of m[r1] before the swap
 * @author amit
 */
public class InvalidateCompareAndSwapInstruction extends AbstractInstruction implements MemoryInstructionInterface {
    ...
    public InvalidateCompareAndSwapInstruction() {
    }

    public InvalidateCompareAndSwapInstruction(int instruction) {
        this();
    ...
    public AbstractInstruction getAbstractInstruction(int instruction) {
        return new InvalidateCompareAndSwapInstruction(instruction);
    }
    ...
    public MemoryInstruction getMemoryInstruction(RegisterFile rFile) {
        ...
        MemoryInstruction toRet = MemoryInstruction.CompareAndSwap(address, compareValue, swapValue);
        toRet.setSubType(SubTypes.InvalidateStore);
        return toRet;
    }
    ...
    public static void main(String[] args) {
        ...
        InvalidateCompareAndSwapInstruction c = new InvalidateCompareAndSwapInstruction();
        int instruction = c.assembleInstruction(instr);
        AbstractInstruction.getAbstractInstructionForInstruction(instruction).toString();
        c = (InvalidateCompareAndSwapInstruction) c.getAbstractInstruction(instruction);
